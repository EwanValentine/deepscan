...
var (
	...
	ports = kingpin.Flag("ports", "Port range 8080:8081").String()
...
func New() (*Scanner, error) {
	...
	openResults := make(chan *Result)
	quit := make(chan bool)
	ips := []string{}
	return &Scanner{ipScanResults, portScanResults, openResults, ips, quit}, nil
}
...
func main() {
	...
	case "single":
		...
		start, end, err := convertPortRange(*ports)
		scanner.Single(*ip, start, end)
		output(scanner)
		s.Stop()
...
func output(scanner *Scanner) {
	for {
		select {
		case res := <-scanner.Listen():
